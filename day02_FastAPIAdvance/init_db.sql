...
CREATE
DATABASE IF NOT EXISTS fastapi_demo
...
INSERT INTO book (id, bookname, author, price, publisher)
VALUES (1, 'Python编程从入门到实践', 'Eric Matthes', 89.00, '人民邮电出版社'),
...
-- 结果确认
SELECT COUNT(*) AS 书籍总数, MAX(price) AS 最高价, MIN(price) AS 最低价, AVG(price) AS 平均价
FROM book;
SELECT id, bookname, author, price, publisher
FROM book
ORDER BY id;
